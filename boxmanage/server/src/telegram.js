...
async function sendTelegram(text) {
  const enabled = getSetting(ENABLED_KEY, '0') === '1';
...
async function checkItemAlert(itemId, previousQty) {
  const item = db.prepare(`
    ...
    WHERE i.id = ?
  `).get(itemId);
  if (!item || item.alert_threshold === null || item.alert_threshold === undefined) return;
...
async function sweepLowStock() {
  const items = db.prepare('SELECT id FROM items WHERE alert_threshold IS NOT NULL').all();
